...
function Home() {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await fetch('http://localhost:8000/allquestions');
      if (!response.ok) {
        throw new Error('Failed to fetch questions');
      }
      const data = await response.json();
      setmongodata(data.questions);
      console.log(data.questions); // Logging the fetched questions to the console
    } catch (error) {
      console.error('Error:', error.message);
    }
  };
    ...
    <div className="main">
      ...
      <div className="Home">
        ...
        <div className="Posts-container">
          {mongodata?.length > 0 &&
           mongodata?.map((e, index) => (
              <div className="Post">
                <div className="Post-header">
                  <div className="ProfileIcon" style={{ cursor: "pointer",position:"relative" }}  onClick={() => {navigate(`/profile/${e?.user_id?._id}`)}}>
                   
                    <img src={e?.user_id?.image} alt="icon" className="image"   />
                {e?.user_id?.occupation=="Teacher"&&<span class="goldBG goldText fix">Professor</span>}    
                  </div>
