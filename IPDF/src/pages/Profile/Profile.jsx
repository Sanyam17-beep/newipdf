...
function Profile() {
  ...
  async function handleImageChange(e) {
    ...
    try {
      ...
      setUserImg(data.secure_url); // Set the image URL to state
  ...
  const updateUser = async (event) => {
    ...
    if (Name || Email || passwo || bio || occ || userImg) {
      ref.current.continuousStart();
      await fetch(`http://localhost:8000/updateUser/${JSON.parse(user).id}`, {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          ...
          bio: bio,
          occupation: occ,
          image: userImg != "" ? userImg : JSON.parse(user).image,
        }),
      })
        .then((res) => {
    ...
    const newuser = {
      img: userImg ? userImg : JSON.parse(user).img,
      id: JSON.parse(user).id,
      name: Name != "" ? Name : JSON.parse(user).name,
      email: Email != "" ? Email : JSON.parse(user).email,
      password: passwo ? passwo : JSON.parse(user).password,
      bio: bio ? bio : JSON.parse(user).bio,
  ...
  return (
    <div className="main">
      ...
      <Modal
        ...
        onRequestClose={handleCloseDialog1}
        className="formModal"
      ...
      >
        ...
        <div style={{ fontSize: "12px" }}>
          <strong>
            Note: To Get access to upload video solution verify with password
          </strong>
        </div>
        <input
          type="password"
        ...
        <div className="button-container">
          <button onClick={handleCloseDialog1} className="cancel modbut">
            Cancel
          </button>
          <button onClick={(e)=>handleSubmitDialog1(e)} className="submit modbut" style={{cursor:"pointer"}}>
            Verify
          </button>
        </div>
      </Modal>
          ...
          <div className="Profile-description">
              ...
              <>
                ...
                <div style={{ fontSize: "25px" }}>
                  {userProfile?.name}
                  {userProfile?.occupation == "Teacher" && (
                    <span class="goldBG goldText siz">Professor</span>
                  )}
                </div>
                <div style={{ color: "#f0f8ff57" }}>
                  <i>{userProfile?.bio}</i>
                </div>
                <div style={{ fontSize: "18px" }}>{userProfile?.occupation=="Teacher"?"Teacher":"Student"}</div>
                <div
                ...
                >
                  ...
                  >
                    Posts{" "}
                    <div style={{ fontSize: "25px" }}>{userQuestion?.length}</div>
                  </div>
              ...
              <>
                <div className="radio-toolbar">
                  ...
                  <input
                    ...
                    onChange={handleRadioChange}
                  />
                  <label htmlFor="radio3" onClick={handleOpenDialog1}>
                    Teacher
                  </label>
                </div>
                <input
                  ...
                  <div className="upvote-downvote">
                    <div className="Vote-reply-container">
                      <div className="vote">
                        ...
                        <div className="vote-count">{e.votes}</div>
                        <div
                          className="downvote"
                          style={{ cursor: "pointer" }}
                      ...
                      <div className="reply">Reply: {e.replies?.length}</div>
                    </div>
