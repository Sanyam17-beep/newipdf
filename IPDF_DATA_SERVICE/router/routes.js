...
router.put('/updateUser/:id', async (req, res) => {
    ...
    const { username, email, password,bio,occupation,image } = req.body;
    await User.findByIdAndUpdate(uid, { name: username, email: email, password: password,bio: bio,occupation: occupation,image:image })
        .then((re) => {
            console.log(re);
...
router.post("/setImage/:id",async(req,res)=>{
  ...
  router.get('/allquestions', async (req, res) => {
    try {
        const allQuestions = await Questions.find().populate("user_id");
...
router.get('/questions/:questionId', async (req, res) => {
    ...
    await Questions.findById(question).populate('user_id').populate('replies').then((re) => {
...
router.get("/getUserProfile/:uid",async(req,res)=>{
    ...
    try{
        ...
        const allquesofUser = await Questions.find({user_id:uid}).populate('replies').populate('user_id');
